...
public class ReceiveServer {
	public static void main(String[] args) throws Exception {
		...
		new Thread(new Runnable() {		
			@Override
			public void run() {
				System.out.println("ReceiveServer send");
				String str = null;
				...
				try {
					while((str = br.readLine()) != null) {
						...
						try {
							dp = new DatagramPacket(strByte,strByte.length,InetAddress.getByName("localhost"),9900);
		...
		new Thread(new Runnable() {			
			@Override
			public void run() {
				System.out.println("ReceiveServer receive");
				while(true) {
